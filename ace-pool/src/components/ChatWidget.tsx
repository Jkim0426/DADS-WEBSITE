...
export default function ChatWidget() {
  ...
  // Scroll to bottom on new bubbles / thinking state
  useEffect(() => {
    scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: "smooth" });
  }, [bubbles, thinking]);

  // Nudge after 10s if unopened
  useEffect(() => {
  ...
  const pick = (option: ChatOption) => {
    ...
    // Push the user's choice as a bubble
    ...
    // Brief "thinking" pause, then show answer
    setThinking(true);
        ...
        {open && (
          ...
          >
            ...
            >
              ...
              {/* Spacer so content isn't hidden by options panel */}
              <div className="h-2" />
            </div>

            {/* Option buttons — pinned above footer */}
            {!thinking && (
              <motion.div
                key={nodeId}
                initial={{ opacity: 0, y: 6 }}
